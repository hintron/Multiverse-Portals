...
public class PortalPlayerSession {
    ...
    public long getRemainingTeleportCooldown(Date time) {
        long cooldown_end_ms = this.lastTeleportTime.getTime() + this.plugin.getCooldownTime();
        long time_ms = time.getTime();
        if (cooldown_end_ms > time_ms) {
            return cooldown_end_ms - time_ms;
        } else {
            return 0;
        }
    }

    public String getCooldownMessage(long cooldown_ms) {
